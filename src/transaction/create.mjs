 ...
 * @returns 
 */
export const createTransaction = async (fromAddress, value, balance, fee, wif, toAddress, script, index, txid, hex) => {
  try {
    console.log("Parameters: %o", { fromAddress, value, toAddress, script, txid, hex });
    ...
    tx.from(utxo).to(toAddress, value).change(fromAddress).fee(fee).sign(privateKey);
